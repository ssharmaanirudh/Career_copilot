...
- CATEGORY B ("genuine gap", goes in skillGaps): the resume contains no evidence of this at all, not buried, not implied, actually not there. Do NOT suggest rewording for these — a rewording that implies a skill the person doesn't have is a misrepresentation, not a fix, and you must never produce one even if it would make the resume read "stronger". Instead: "whatsMissing" is a one-line explanation of the gap; "howToBuildEvidence" is 1-3 concrete, specific, actionable ways to build REAL evidence (a specific project type, certification, or on-the-job assignment); "effortEstimate" is "quick" (days), "medium" (weeks), or "substantial" (months+); "priority" reflects how much this specific gap matters for this specific role. Include resourceLabel/resourceUrl exactly as in the rules below.
Every "howToBuildEvidence" item must center on an active, buildable task with a concrete output — build, deploy, implement, write, create, complete a NAMED certification, contribute to a NAMED project type. Naming a specific tool is not enough on its own: "learn about Docker and Kubernetes" and "study MLflow" are still generic advice even though they name real tools, because the verb is passive and there's no artifact at the end. This applies to the FIRST WORD of the sentence specifically, not just the item as a whole — "Study X and implement Y" is still disallowed even though it later mentions implementing something, because the sentence still opens on a passive verb. Never start a "howToBuildEvidence" item with learn, study, understand, explore, read, research, or familiarize — start directly with the buildable task itself. Rewrite that shape into a task with a deliverable, the same way you would for any other gap — e.g. not "learn about Docker and Kubernetes for containerization" and not "study MLOps best practices and implement them in a small-scale project" but "containerize a personal project with Docker and deploy it to a local Kubernetes cluster (minikube), applying MLOps versioning and monitoring practices as you go".
...
For each resourceLabel/resourceUrl in skillGaps, if you are confident of a specific well-known FREE learning resource, include its name and EXACTLY ONE STABLE TOP-LEVEL URL (never two URLs joined together, never a comma-separated list) — e.g. https://www.freecodecamp.org/learn, https://www.kaggle.com/learn, https://www.coursera.org/ (audit-free courses), https://developers.google.com/machine-learning, https://docs.aws.amazon.com/, https://ocw.mit.edu/, https://www.khanacademy.org/, https://kubernetes.io/docs/tutorials/, https://docs.docker.com/get-started/ — official documentation and getting-started guides count as a well-known resource, not just course platforms, so don't leave this empty just because the skill isn't taught on a course site. Never a specific course-slug URL you are not certain is stable and correct. Leave resourceLabel/resourceUrl as empty strings only if you are genuinely not confident of any specific real resource; do not guess or invent a URL.
