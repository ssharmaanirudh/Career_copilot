...
HARD RULE, no exceptions — hedging language in an item's OWN wording always overrides marker-pair position, heading, prominence, or repetition: if the specific item you are classifying is itself qualified by hedging/soft language — "preferred," "nice to have," "a plus," "bonus," "exposure to," "familiarity with," "desired," "ideally," "would be advantageous," or equivalent phrasing — it is desirable, never essential. This applies even when the item sits inside a "[STRUCTURED REQUIREMENTS SECTION]" or "[STRUCTURED DUTIES LIST]" marker pair, even when it repeats elsewhere in the JD, and even when surrounding items in the same bulleted list are genuinely essential. Marker-pair authority only establishes that a PLAIN, UNHEDGED item belongs in that section — it never promotes a hedged item past its own wording. Read each item's own sentence before deciding; do not reason from the surrounding list's apparent authority. This is a hard override, not a factor to weigh against other signals — a single hedging word/phrase in the item's own text is dispositive by itself, regardless of how many other essential-sounding signals are also present.

For every requirement, also quote the exact JD phrase (verbatim substring of the JD text, as short as possible while still containing the deciding language) that justifies the type you chose, as "jdQuote" — e.g. for a requirement classified desirable because of hedging language, the quote must contain that hedging language itself, not just the requirement's name. This is a permanent citation safety net so a misclassification is visible immediately next to its own justification, not just theoretically correctable later.
...
Also identify the JD's core technical function — the primary tech stack, core tool, or the stated years-of-experience gate that this role is fundamentally built around. There is at most one such requirement per JD (some JDs, e.g. a generalist or non-technical role, have none). Mark that single requirement's "isCoreRequirement" true; every other requirement gets false. This is the same judgment step 4 uses to decide the hard 15-point cap — decide it once here and reuse it, do not re-derive it in step 4. If you are genuinely torn between two candidates for the core requirement, resolve it yourself: pick your true best answer, list it FIRST among the requirements in requirementsChecklist, and mark only that first one isCoreRequirement true. This matters beyond the checklist itself — only the first isCoreRequirement:true item in your output is treated as authoritative downstream, so every summary/verdict you write anywhere in this response (originalScore.summary, tailoredScore.summary, honestSummary) that references "the" core or primary gap must be about that same first-listed requirement, never a different one, even if a second requirement also felt central to you.
...
- Also populate scoreBreakdown for that pass. Score each of the four dimensions (skillsMatch, experienceMatch, keywordAlignment, overallPresentation) independently, from that specific dimension's own evidence in the resume — never copy matchScore into a dimension, and never default any dimension to the capped matchScore value just because matchScore itself is heavily capped. skillsMatch must not exceed the same essentials-based cap used for matchScore, since skills match is literally what that cap measures. experienceMatch, keywordAlignment, and overallPresentation have NO such ceiling: a resume can be capped to 15 by one unmet core requirement while still having a genuinely strong experience narrative, real quantified results, JD-aligned language, or clean presentation elsewhere — if the evidence for one of those specific dimensions is genuinely strong, score it accordingly, even well above matchScore. It is a real scoring error, not a safe default, for any of these three to end up equal to matchScore or equal to each other without that reflecting an independent judgment that they are, in fact, equally weak.
...
const REQUIREMENT_CHECK_SCHEMA: Schema = {
  ...
  properties: {
    ...
    jdQuote: {
      type: Type.STRING,
      description:
        "Exact quoted JD phrase (verbatim substring) that justifies this requirement's essential/desirable classification. For a desirable item, this must contain the actual hedging language, not just the requirement's name.",
    },
  },
  required: [
    "requirement",
