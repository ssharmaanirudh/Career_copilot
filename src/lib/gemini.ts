...
HARD RULE, same principle as the narrative-mention rule just above, now applied inside marker pairs too: an item's own hedging language — "preferred," "nice to have," "a plus," "bonus," "exposure to," "familiarity with," "desired," "ideally," or equivalent — always makes THAT item desirable, never essential, even inside a "[STRUCTURED REQUIREMENTS SECTION]"/"[STRUCTURED DUTIES LIST]" marker pair, even if it repeats, even next to genuinely essential items in the same list. Marker-pair position only governs a plain, unhedged item; it never promotes a hedged one. Read what each item's own sentence actually says, not the authority of its surroundings — this applies only to the specific hedged item itself, never as a reason to second-guess a neighboring item that has no hedging language of its own.

For every requirement, also quote the exact JD phrase (verbatim substring, as short as possible while still containing the deciding language) that justifies the type you chose, as "jdQuote" — for a desirable call driven by hedging language, the quote must contain that hedging language itself, not just the requirement's name.
