...
/** Not in lib.dom.d.ts — both are real WebGL extension shapes, just untyped. */
...
export function BlackHoleHeroSection({
  ...
  // Keeps the ref current before the browser paints, so the animation loop
  // (which reads props.current lazily, outside React's render cycle) never
  // acts on a stale prop — without writing to the ref during render itself,
  // which React's hooks lint now flags as unsafe under concurrent rendering.
  useLayoutEffect(() => {
    props.current = {
      distance, elevation, azimuth, orbitSpeed, roll, fov, diskInner, diskOuter,
      diskThickness, diskDensity, brightness, spinSpeed, grain, doppler, hotColor,
  ...
  useEffect(() => {
    ...
    const renderer = dbg
      ? String(gl.getParameter((dbg as DebugRendererInfoExt).UNMASKED_RENDERER_WEBGL) || "")
      : "";
    ...
    if (isGL2) {
      ...
      if (hf && cb) texType = (hf as HalfFloatExt).HALF_FLOAT_OES;
      else hdr = false;
    }
    if (!hdr) {
